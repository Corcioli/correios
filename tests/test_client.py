...
@vcr.use_cassette
def test_get_post_info(client, contract, user, posting_card):
    contract.posting_cards = [posting_card]

    user.contracts = [contract]

    def replace_user(contract_number, posting_card_number):
        nonlocal contract, posting_card
        contract.number = contract_number
        posting_card._number = int(posting_card_number)
        return user

    with patch.object(client, 'get_user', new=replace_user):
        ...
        for obj in data.objeto_postal:
            ...
            assert label.posting_card == posting_card
            ...
            assert (
                ...
                (sender.telefone_remetente.text or '')
            )
            ...
            assert (
                ...
                (receiver.email_destinatario.text or '')
            )
            assert label.receiver.name == receiver.nome_destinatario.text
            assert (
                label.receiver.street == receiver.logradouro_destinatario.text
            )
            assert (
                label.receiver.number == receiver.numero_end_destinatario.text
            )
            assert (
            ...
            assert (
                label.receiver.neighborhood ==
                extra_info.bairro_destinatario.text
            )
            assert (
                label.receiver.zip_code == extra_info.cep_destinatario.text
            )
            assert (
                label.receiver.city == extra_info.cidade_destinatario.text
            )
            assert (
                label.receiver.state == extra_info.uf_destinatario.text
            )
            assert (
            ...
            assert package.real_weight == float(
                obj.peso.text.replace(',', '.')
            )
